...
public partial class Report : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            if (metaTag.HttpEquiv.Equals("X-UA-Compatible", StringComparison.CurrentCultureIgnoreCase))
            {
                metaTag.Content = "IE=EmulateIE10";
                break;
            }
        }
        */
        ...
        if (!IsPostBack || Request.Form["txtReportParameters"] != "")
        {
            ...
            {
                ReportViewer1.Height = gc.CookieIntValue(Request.Cookies["AvailableClientHeight"], 400);
            }
            ...
            foreach (XmlNode parameter in parameters)
            {
                ...
                if (parameter.SelectSingleNode("a:Hidden", nsmgr) != null)
                {
                    if (parameter.SelectSingleNode("a:Hidden", nsmgr).InnerText == "true")
                    {
                        if (Session["rqp_HideParameters"].ToString().IndexOf(ParameterName) == -1)
                        {
                            Session["rqp_HideParameters"] = Session["rqp_HideParameters"].ToString() + "|" + ParameterName;
                        }
                    }
                }
            ...
            if (Session["ShowParameters"].ToString() == "Yes")
            {
                ...
                vgParameters.Blocked = Session["rqp_HideParameters"].ToString();
                //vgParameters.Title = "";
            }
