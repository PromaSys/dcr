...
function formSummarize(formID) {
    var jqxhr = $.getJSON("Process_Request.aspx", {
        action: "SummarizeForm",
        fid: formID
    });
}
...
function reportCascade(elem, uncheckHidden) {

    if (uncheckHidden == null || typeof(uncheckHidden) == 'undefined') {
        uncheckHidden = true;
    }
...
function reportDisplayBI(elem) {
    var BIID = $(elem).closest('tr').attr('kfv');
    SetCookie('BIID', BIID, 1);
    location.href = 'bi_report';
    //window.open('bi_report.aspx');

    /*
    $('#BIID').val(BIID);
    $("form:first").attr('target', 'bi_report');
    $("form:first").attr('action', 'bi_report');
    $("form:first").submit();
    */
    ...
    */
    /*
    $('#BIID').val(BIID);
    $("form:first").attr('type', 'POST');
    $("form:first").attr('action', 'bi_report.aspx');
    $("form:first").submit();
    */
    /*
    $.ajax({
    ...
    */
    ...
    //window.open('bi_report.aspx?BIID=' + encodeURIComponent(BIID), 'bi_report');
}
